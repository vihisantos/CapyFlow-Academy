        ...
        <div className="min-h-screen bg-[#020617] text-white p-8 pb-32">
            <div className="max-w-6xl mx-auto space-y-12">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end pb-8 border-b border-white/5 space-y-6 md:space-y-0 relative">
                    <Link href="/" className="absolute top-[-30px] left-0 text-sm text-gray-500 flex items-center hover:text-primary transition-colors">
                        <ArrowLeft size={16} className="mr-1" /> Voltar para Início
                    </Link>
                    <div className="flex items-center space-x-6 pt-4">

                        <img src={user.photoURL} className="w-24 h-24 rounded-full border-2 border-primary/50 shadow-[0_0_30px_rgba(var(--primary),0.2)]" />
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                <h1 className="text-4xl font-bold tracking-tight text-white">{user.displayName}</h1>
                                {isPro && (
                                    <span className="bg-accent/20 text-accent border border-accent/30 px-2 py-1 rounded-md text-xs font-bold flex items-center gap-1 uppercase tracking-wider">
                                        <Crown size={12} className="fill-current" /> Pro
                                    </span>
                                )}
                            </div>
                            <div className="flex items-center space-x-3 text-sm text-gray-400">
                                <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 uppercase tracking-widest text-xs font-bold text-primary">
                                    Nível {currentLevel}
                                </span>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        <Link href="/arena" className="px-8 py-3 cyber-btn rounded-xl font-bold flex items-center space-x-2 text-background">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-sm font-medium">
                        <span className="text-primary">XP Atual: {xpInLevel}</span>
                    ...
                    <div className="h-4 w-full bg-white/5 rounded-full overflow-hidden border border-white/5 relative">
                        <motion.div
                            ...
                            className="h-full bg-linear-to-r from-primary to-secondary shadow-[0_0_20px_rgba(var(--primary),0.4)]"
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <StatCard icon={<Trophy className="text-accent" />} label="Sessões Completas" value={stats?.totalSnippetsCompleted || 0} />
                    <StatCard icon={<Zap className="text-primary" />} label="Melhor WPM" value={Math.max(...sessions.map(s => s.wpm), 0)} />
                    <StatCard icon={<Activity className="text-secondary" />} label="Precisão Média" value={Math.round(sessions.reduce((acc, s) => acc + s.accuracy, 0) / (sessions.length || 1)) + "%"} />
                    <StatCard icon={<Clock className="text-gray-400" />} label="Tempo de Treino" value={Math.round((stats?.totalTimeSpent || 0) / 60) + " min"} />
                        ...
                        {sessions.length === 0 ? (
                            ...
                            sessions.map((session) => (
                                <motion.div
                                    ...
                                    className="p-4 glass rounded-xl border border-white/5 flex justify-between items-center hover:border-primary/20 transition-colors"
                                >
                                    ...
                                    <div className="flex space-x-6 text-right">
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-primary">{session.wpm}</div>
                                        </div>
                                        <div>
                                            <div className="text-xs text-gray-500 uppercase">XP</div>
                                            <div className="text-xl font-bold text-accent">+{session.xpEarned}</div>
                                        </div>
                                    </div>
                                </motion.div>
                            ))
